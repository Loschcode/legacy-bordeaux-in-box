@section('content')
  ...
  <div class="container profile-orders-section">
    ...
    <div class="col-md-10">
      ...
      <!--<h1 class="title">Abonnement #{{$profile->id}}</h1>-->
...
<h1>Mon abonnement à {{$profile->box()->first()->title}}</h1>
  ...
  <tbody>
    ...
    @foreach ($payments as $payment)
      ...
      <tr>
        ...
        <th>
        @if ($payment->amount > 0)
          <a href="{{url('/profile/bill/' . $payment->bill_id)}}" target="_blank">Accéder à ma facture</a> | <a href="{{url('/profile/download-bill/' . $payment->bill_id)}}" target="_blank">Télécharger ma facture</a>
        @endif
        </th>
