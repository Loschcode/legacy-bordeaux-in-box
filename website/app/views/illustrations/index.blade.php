...
@else
	...
	@if ($previous_article !== NULL)
		...
		<a href="{{url('illustrations/index/'.$previous_article->id)}}">Précédent</a>

	@endif

	@if ($next_article !== NULL)
	
		<a href="{{url('illustrations/index/'.$next_article->id)}}">Suivant</a>
		
	@endif


	{{$image_article->title}}<br />

	<img src="{{ url($image_article->image->full) }}">
