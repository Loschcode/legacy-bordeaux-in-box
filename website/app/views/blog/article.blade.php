...
{{$blog_article->content}}<br />

<img src="{{ url($blog_article->thumbnail->full) }}"><br />

@if ($previous_article !== NULL)

	<a href="{{url('blog/article/'.$previous_article->id)}}">&lt; {{$previous_article->title}}</a>

@endif

@if ($next_article !== NULL)

	<a href="{{url('blog/article/'.$next_article->id)}}">{{$next_article->title}} &gt;</a>
