...
<br />
  ...
  @else
    ...
    {{{ $errors->first('billing_city') }}}<br />
    @endif

    {{ Form::label("billing_city", "Ville") }}
    {{ Form::text("billing_city", (Input::old("billing_city")) ? Input::old("billing_city") : $user->city) }}<br />


    @if ($errors->first('billing_zip'))
    {{{ $errors->first('billing_zip') }}}<br />
    @endif

    {{ Form::label("billing_zip", "Code postal") }}
    {{ Form::text("billing_zip", (Input::old("billing_zip")) ? Input::old("billing_zip") : $user->zip) }}<br />


    @if ($errors->first('billing_address'))
    {{{ $errors->first('billing_address') }}}<br />
    @endif

    {{ Form::label("billing_address", "Adresse") }}<br />
    {{ Form::textarea("billing_address", (Input::old("billing_address")) ? Input::old("billing_address") : $user->address) }}<br />

  @endif

  <h2>Informations de livraison</h2>

  <a href='#' id="copy_billing_informations">Copier les informations de facturation</a><br /><br />

  @if ($errors->first('destination_first_name'))
  {{ $errors->first('destination_first_name') }}<br />
  @endif

  {{ Form::label("destination_first_name", "Prénom") }}
  {{ Form::text("destination_first_name", ($order_building->destination_first_name) ? $order_building->destination_first_name : Input::old("destination_first_name")) }}<br />

  @if ($errors->first('destination_last_name'))
  {{ $errors->first('destination_last_name') }}<br />
  @endif

  {{ Form::label("destination_last_name", "Nom") }}
  {{ Form::text("destination_last_name", ($order_building->destination_last_name) ? $order_building->destination_last_name : Input::old("destination_last_name")) }}<br />

  <br />

  @if ($errors->first('destination_city'))
  {{{ $errors->first('destination_city') }}}<br />
  @endif

  {{ Form::label("destination_city", "Ville") }}
  {{ Form::text("destination_city", ($order_building->destination_city) ? $order_building->destination_city : Input::old("destination_city")) }}<br />


  @if ($errors->first('destination_zip'))
  {{{ $errors->first('destination_zip') }}}<br />
  @endif

  {{ Form::label("destination_zip", "Code postal") }}
  {{ Form::text("destination_zip", ($order_building->destination_zip) ? $order_building->destination_zip : Input::old("destination_zip")) }}<br />


  @if ($errors->first('destination_address'))
  {{{ $errors->first('destination_address') }}}<br />
  @endif

  {{ Form::label("destination_address", "Adresse") }}<br />
  {{ Form::textarea("destination_address", ($order_building->destination_address) ? $order_building->destination_address : Input::old("destination_address")) }}<br />


  <a href="{{url('/order/choose-frequency')}}">Retour au choix de la fréquence</a>
