...
<h2>Choisis ta fréquence de livraison</h2>

{{ Form::open() }}

	Prochaine série : {{$next_series->first()->delivery}}<br /><br />

@if ($order_preference->gift == TRUE)

	@foreach ($delivery_prices as $delivery_price)

		{{ Form::label($delivery_price->id, $delivery_price->readableFrequency() . ' ('. $delivery_price->unity_price.'€)') }}
		{{ Form::radio('delivery_price', $delivery_price->id, ($order_preference->frequency == $delivery_price->frequency) ? true : Input::old($delivery_price->id), array('id' => $delivery_price->id))}}<br />
...
@else
	...
	@foreach ($delivery_prices as $delivery_price)
		...
		@if ($delivery_price->frequency == 1)
		{{ Form::label($delivery_price->id, $delivery_price->readableFrequency() . ' ('. $delivery_price->unity_price.'€)') }}
		@else
		{{ Form::label($delivery_price->id, $delivery_price->readableFrequency() . ' ('. $delivery_price->unity_price.'€ par mois)') }}
		@endif
...
<a href="{{url('/order/box-form')}}">Retour à la personnalisation</a>
