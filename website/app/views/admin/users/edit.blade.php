...
@section("content")
  ...
  {{ Form::text("email", ($user->email) ? $user->email : Input::old("email")) }}
  <br />
  ...
  {{ Form::text("password", '') }} <em>(Laisser vide si inchangé)</em>
  <br />
  ...
  {{ Form::select('role', $roles_list, $user->role) }}
  <br />
  <br />
  ...
  {{ Form::submit("Editer cet utilisateur") }}

  {{ Form::close() }}

  <br /><br />
  Note : si l'utilisateur possède des abonnements, l'adresse de facturation des abonnements éditables sera également modifiée.
