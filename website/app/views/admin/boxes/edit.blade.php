...
@section("content")
  ...
  {{ Form::text("title", (Input::old("title")) ? Input::old("title") : $box->title) }}
  <br />

  @if ($errors->first('image'))
  {{{ $errors->first('image') }}}<br />
  @endif
  {{ Form::label("image", "Image (laisser vide si vous ne voulez pas la changer)") }}
  {{ Form::file('image') }}

  <br /><br />

  {{ Form::submit("Mettre à jour cette box (elle sera désactivée par défaut)") }}
