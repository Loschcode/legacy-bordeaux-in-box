(function() {
  ...
  this.Payment = (function() {
    function Payment(stripeKey) {
      this.displayDefault = __bind(this.displayDefault, this);
      this.afterPayment = __bind(this.afterPayment, this);
      this.initStripe = __bind(this.initStripe, this);
    ...
    Payment.prototype.displayLoading = function(message) {
      return $('#trigger-payment').prop('disabled', true).addClass('cart-buy-disabled').html('<i class="fa fa-spinner fa-spin"></i> ' + message);
    };

    Payment.prototype.displayDefault = function() {
      return $('#trigger-payment').prop('disabled', false).removeClass('cart-buy-disabled').html('<i class="fa fa-credit-card"></i> Procéder au paiement sécurisé');
    };
